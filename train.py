...
parser.add_argument('--log_freq', type=int, default=10, help='log freq')
...
if args.resume:
    ...
    all_saved_ckpts = [x for x in all_saved_ckpts if 'best' not in x]
...
# dataset, dataloader
train_dataset = CustomDataset(args.trainlist, True)
...
def train():
    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            scalar_outputs = train_sample(sample)
            if (not is_distributed) or (dist.get_rank() == 0):
        ...
        if (not is_distributed) or (dist.get_rank() == 0):
            ...
            save_scalars(logger, 'fulltrain', avg_train_scalars, len(TrainImgLoader) * (epoch_idx + 1))
            print("avg_train_scalars", avg_train_scalars)
            nowerror = avg_test_scalars["epe"]
            if  nowerror < error :
                bestepoch = epoch_idx
                error = avg_test_scalars["epe"]
...
def train_sample(sample):
    ...
    weight_loss = [0.5, 0.7, 1.0 , 0.6] # hmsmnet 
...
#本部分结束
    ...
    err_mask = err_map > 3
    err_disps = torch.sum(err_mask.astype('float32'))
    nums = torch.sum(mask)
    d1 = err_disps / nums
